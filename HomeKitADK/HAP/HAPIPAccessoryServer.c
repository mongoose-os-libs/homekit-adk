...
static void handle_http(HAPIPSessionDescriptor* session) {
    ...
    if ((content_length <= session->inboundBuffer.position) &&
        ...
        size_t requestLen = session->httpReaderPosition + content_length;
        HAPLogBufferDebug(
                &logObject,
    ...
    if (session->securitySession.isSecured || kHAPIPAccessoryServer_SessionSecurityDisabled) {
        ...
        if (numReadContexts > 0) {
            ...
            int r = handle_characteristic_read_requests(
                    session, kHAPIPSessionContext_EventNotification, readContexts, numReadContexts, &data_buffer);
            (void) r;
            ...
            if (content_length <= session->outboundBuffer.limit - session->outboundBuffer.position) {
                ...
                err = HAPIPAccessoryProtocolGetEventNotificationBytes(
                        HAPNonnull(session->server), readContexts, numReadContexts, &session->outboundBuffer);
                HAPAssert(!err && (session->outboundBuffer.position - mark == content_length));
                HAPIPByteBufferFlip(&session->outboundBuffer);
                HAPLogBufferDebug(
                        &logObject,
...
static void ReadInboundData(HAPIPSessionDescriptor* session) {
    ...
    size_t numBytes = 0;
    size_t maxBytes = b->limit - b->position - 1;
    ...
    HAPLogDebug(
            &logObject,
            "BUF %p: read %u pos %u lim %u cap %u",
            b,
            (unsigned) numBytes,
            (unsigned) (b->position + numBytes),
            (unsigned) b->limit,
            (unsigned) b->capacity);
...
static void engine_init(HAPAccessoryServerRef* server_) {
    ...
    HAPLogDebug(
            &logObject,
            "Actual sizes: HAP_SESSION_SIZE = %lu, HAP_IP_SESSION_SIZE = %lu, HAP_ACCESSORY_SERVER_SIZE = %lu",
            (unsigned long) sizeof(HAPSession),
            (unsigned long) sizeof(HAPIPSessionDescriptor),
            (unsigned long) sizeof(HAPAccessoryServer));
    HAPLogDebug(
            &logObject,
